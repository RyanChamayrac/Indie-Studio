...
Core::Core(IrrlichtDevice *const window) : _window(window)
{
    ...
    this->button2 = this->driver->getTexture("./resources/PLAY.png");
...
void Core::run()
{
    ...
    while (_window->run()) {
        ...
        this->driver->draw2DImage(this->button2, position2d<s32>(748, 200), rect<s32>(0, 0, 425, 77), 0, SColor(255, 255, 255, 255), true);
        this->smgr->drawAll();
        this->env->drawAll();
